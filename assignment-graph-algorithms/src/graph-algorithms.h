...
std::list<value_type<T>> dijkstrasAlgorithm(const WeightedGraph<T>& graph, vertex_type<T> initial_node, vertex_type<T> destination_node)
{
    ...
    */
    ...
    for (auto &item : graph){
        q.push(item.first); //push the keys of every item in the graph
    }
    ...
    */
    while (q.size() > 0){
        auto u = q.top(); //gets top of min queue
        if(distances[u] == infinity<T>()){
            break;
        }
        q.pop(); //pops the top of min queue aka the value we just used
        
        if (s.find(u) == s.end()){ //if we can't find the element
            s.insert(u);
        }
...
std::list<value_type<T>> topologicalSort(const WeightedGraph<T>& graph)
{
    ...
    while (q.size() > 0){//queue length greater than 0
        size_type<T> v_key = q.front(); //v_key is the key from the queue
        l.push_back(v_key);
        q.pop();
        auto adj_list = graph.at(v_key);

        for (auto vertex : adj_list){ //for each vertex in v's adjacency list
            indegrees[vertex.first] = indegrees[vertex.first]-1;
